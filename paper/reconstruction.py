...
# Anchor all I/O to the repository root so the module works regardless of the
# caller's working directory (paper/reconstruction.py -> parents[1] is the root).
_ROOT = Path(__file__).resolve().parents[1]
_DATA = _ROOT / 'data' / 'phantoms_from_paper'
RESULTS_DIR = str(_ROOT / 'paper' / 'figures') + os.sep
CACHE_FILE = str(_ROOT / 'cache' / 'multiresolution_results.pkl')
...
# Two-channel preconditioner: the LF dual receives a sigma_lo_scale-times
# larger step than the HF/TV/L1 duals. The paper-config value 4.0 is the
# scaling that produced the abstract's reported iter-500 RMSE numbers.
sigma_lo_scale = 4.0
...
def run_reconstruction_for_mfact(mfact, snapshot_iters=None,
    ...
    if phantom_override is not None:
        ...
        phantom1 = load(str(_DATA / "Phantom_Adipose.npy"))[imagenumber]
        phantom2 = load(str(_DATA / "Phantom_Fibroglandular.npy"))[imagenumber]
        phantom3 = load(str(_DATA / "Phantom_Calcification.npy"))[imagenumber]
