...
\end{equation}
where $R[c]=\Lambda^{1/2}\,\mathrm{Hann}^{1/2}(c)$ acts along detector channels: $\Lambda^{1/2}$ is the square-root ramp (the Fourier multiplier $|u|^{1/2}$) and $\mathrm{Hann}^{1/2}(c)$ a square-root Hanning window of cutoff $c$. The ramp preconditions the fidelity by flattening the $1/|u|$ spectral decay of $X^\top X$, and both factors vanish at DC, so $R[c]^\top R[c]=X^\top\big(|u|\,\mathrm{Hann}(c)\big)X$ has strongly suppressed low-frequency singular values. This de-emphasis slows the convergence of the near-DC image content, but---as we exploit below---it is also what makes the near-DC band spectrally well separated from the high-frequency band.
...
\end{cases}
\end{equation}
Both filters carry the same square-root ramp as the single-channel fidelity and differ only in their Hanning window: $R_{\mathrm{hi}}=\Lambda^{1/2}\mathrm{Hann}^{1/2}(c)$ preserves the original cutoff, while $R_{\mathrm{lo}}=\Lambda^{1/2}\mathrm{Hann}^{1/2}(c_{\mathrm{lo}})$ with $c_{\mathrm{lo}}\ll c$ is narrow and supported near DC, the pair being complementary so that $R_{\mathrm{hi}}^2+R_{\mathrm{lo}}^2\approx R[c]^2$ over the inversion-relevant passband. The shared ramp is essential to the construction: by suppressing the near-DC singular values it keeps the low band's contribution to the weighted operator norm small, which is exactly the band separation that Remark~\ref{rmk:tight} relies on; without it the low and high bands overlap, the separation is lost, and the certified low-frequency amplification no longer pays off. The low-frequency tolerance is chosen slightly looser, $\varepsilon_{\mathrm{lo}}>\varepsilon_{\mathrm{hi}}$, to permit mild model mismatch at DC while still driving large-scale bias down.
