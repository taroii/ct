...
def reconstruct_slice_two_channel(param_dict, sinodata, phimage, verbose=False):
    ...
    """
    # Extract parameters
    beta_val = param_dict.get('beta', 5.0)
    ...
    iter_opt = 200 if itermax > 200 else itermax  # Use fewer iterations during optimization
    ...
    for itr in range(1, iter_opt+1):
        ...
        yim = beta_val*ptil1/maximum(sqrt(ptil1**2), beta_val)
...
def optimize_parameters_for_victre():
    ...
    param_bounds = {
        'beta': (0.1, 2.0),            # L1 sparsity penalty (lower for dense tissue)
        'cutoff_lo': (2.0, 16.0),      # Two-channel frequency split
        'sigma_scale': (1.0, 8.0),     # Two-channel step size scaling
        'eps_ratio': (0.8, 2.0)        # Two-channel eps_lo/eps_hi ratio
    }
    ...
    # Extract RMSE values from history for saving (history contains tuples of (params, rmse))
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("Optimized parameters saved to: ../results/raw_victre/ecp_optimized_params.npz")
    print("\nKey findings:")
    print(f"  beta (L1 sparsity): {best_params.get('beta', 'N/A'):.3f} (applies to BOTH single and two-channel)")
    print(f"  cutoff_lo: {best_params.get('cutoff_lo', 'N/A'):.3f} (two-channel only)")
    print(f"  sigma_scale: {best_params.get('sigma_scale', 'N/A'):.3f} (two-channel only)")
    print(f"  eps_ratio: {best_params.get('eps_ratio', 'N/A'):.3f} (two-channel only)")