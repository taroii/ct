...
# Note: ECP parameter optimization is done separately via optimize_victre_params.py
# Optimized parameters are loaded from ecp_optimized_params.npz if available
import argparse
import sys
...
# ============================================================================
# LOAD PRE-OPTIMIZED PARAMETERS (if available)
# ============================================================================
...
if args.no_optimized:
    ...
    try:
        ...
        if 'best_params' in saved:
            opt_params = saved['best_params'].item()
            # Apply optimized beta to CONFIG (affects BOTH single and two-channel)
            if 'beta' in opt_params:
                CONFIG['beta'] = opt_params['beta']
                print(f"Loaded optimized beta={CONFIG['beta']:.3f} from {opt_params_file.name}")
            # Apply two-channel specific parameters
            if 'cutoff_lo' in opt_params:
